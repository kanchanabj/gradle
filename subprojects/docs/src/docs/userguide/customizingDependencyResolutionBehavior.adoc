...
There are a number of ways that you can influence how Gradle resolves dependencies. All these mechanism offer an API to define a reason for why they are used. It is recommended to make use of this to make dependency resolution results more understandable. If any customization influenced the resolution result, the provided reason will show up in <<sec:identifying_reason_dependency_selection,dependency insight>> reports.
