...
When you declare a <<declaring_dependencies,dependency>> dependency or a <<sec:dependency_constraints,dependency constraint>>, you can provide a custom reason for the declaration. This makes your build script and the dependency insight report easier to comprehend.

++++
<sample id="dependencyReason" dir="userguide/dependencyManagement/inspectingDependencies/dependencyReason" title="Giving a reason why a certain modules version is used in a dependency declaration">
